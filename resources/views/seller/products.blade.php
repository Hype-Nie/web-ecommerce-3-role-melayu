        ...
        <form action="{{ route('seller.products.store') }}" method="POST" enctype="multipart/form-data">@csrf
            <div class="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div>
                    ...
                    <input type="file" name="images[]" multiple accept="image/*" class="input-styled !py-2 !text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100">
        ...
        <form id="edit-product-form" method="POST" enctype="multipart/form-data">@csrf @method('PUT')
            <div class="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    ...
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Stok</label><input type="number" name="stock" id="edit-p-stock" class="input-styled" required></div>
                </div>
                <div class="grid sm:grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Kategori</label>
                    ...
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Harga Lama (RM)</label><input type="number" name="old_price" id="edit-p-old_price" step="0.01" class="input-styled"></div>
                </div>
        ...
        .then(d => {
            ...
            body.innerHTML = `
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div><p class="text-gray-400 text-xs">Harga Lama</p><p class="font-medium">${d.old_price ? 'RM ' + parseFloat(d.old_price).toFixed(2) : '-'}</p></div>
                    <div><p class="text-gray-400 text-xs">Stok</p><p class="font-medium">${d.stock}</p></div>
                    <div><p class="text-gray-400 text-xs">Status</p><p><span class="badge ${d.is_active ? 'badge-success' : 'badge-danger'}">${d.is_active ? 'Aktif' : 'Tidak Aktif'}</span></p></div>
...
function editProduct(id) {
    fetch(`/penjual/produk/${id}`, { headers: { 'Accept': 'application/json' }})
        ...
        .then(d => {
            ...
            document.getElementById('edit-p-stock').value = d.stock;
