...
<nav class="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
        ...
        <div class="flex items-center justify-between h-16 lg:h-20">
            ...
            <div class="hidden md:flex flex-1 max-w-xl mx-8">
                <div class="relative w-full">
                    ...
                    <input type="text" placeholder="Cari produk, kategori, atau jenama..." class="w-full pl-12 pr-4 py-2.5 rounded-full bg-gray-100 border border-transparent text-sm transition-all duration-300 outline-none focus:bg-white focus:border-primary-400 focus:ring-4 focus:ring-primary-500/10 search-global">
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('cart') }}" class="relative p-2.5 rounded-xl hover:bg-gray-100 transition-colors" id="nav-cart-btn">
                    ...
                    @auth
                        ...
                        @if($cartCount > 0)
                        <span class="absolute -top-0.5 -right-0.5 w-5 h-5 bg-primary-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center animate-bounce-in">{{ $cartCount }}</span>
                        @endif
                    @endauth
                </a>
    ...
    <div id="mobile-menu" class="hidden flex-col md:hidden bg-white border-t border-gray-100 animate-fade-in-down">
        <div class="p-4">
            <div class="relative">
                <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                <input type="text" placeholder="Cari produk..." class="w-full pl-12 pr-4 py-3 rounded-xl bg-gray-100 text-sm outline-none focus:bg-white focus:ring-2 focus:ring-primary-500/20">
            </div>
        </div>
